...
import { $axiosCoordinator, $axiosProofGenerator, $axiosOpenApi } from '@/lib/api';
...
export const handler: RequestHandler<L2TxReqDto, null> = async function (req, res): Promise<BaseResponse<string>> {
    ...
    const rs0 = await $axiosOpenApi.post<BaseResponse<any>>('/existence/data-roots', [dataRoot]).then(r => {
    ...
    const rs = await $axiosOpenApi.post<BaseResponse<any>>('/existence/nullifiers', [nullifier1, nullifier2]).then(r => {
        return new Map(Object.entries(r.data.data));
    })
