...
async function fetchActionsAndEvents() {
    ...
    try {
        ...
        /* no need reversed again from 20231022, since coldStar has commented the 
        ...
        */
        ...
        if (newActionList[newActionList.length - 1].hash != zkAppActionStateArray![0].toString()) {
            logger.info(`the hash attached to latest action is NOT aligned with onchainActionStateArray[0]`);
            throw new Error('the hash attached to latest action is NOT aligned with onchainActionStateArray[0]');
        }
        ...
        if (calcActionHash.toString() != zkAppActionStateArray![0].toString()) {
            throw new Error('calcActionHash is NOT aligned with onchainActionStateArray[0]');
        }
