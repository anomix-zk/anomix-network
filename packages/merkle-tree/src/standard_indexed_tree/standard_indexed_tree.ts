...
const hashEncodedTreeValue = (leaf: LeafData, hasher: Hasher) => {
  return hasher.compressInputs([leaf.value, leaf.nextValue, leaf.nextIndex]);
};
...
export class StandardIndexedTree extends TreeBase implements IndexedTree {
  ...
  public getLeafValue(
    ...
    return Promise.resolve(leaf.value);
  }
   ...
   */
  public async getPureLeafValue(index: bigint, includeUncommitted: boolean) {
  ...
  public getLatestLeafDataCopy(
    ...
    if (this.leaves[index]) {
      log(
        `this.leaves[${index}]: ${JSON.stringify({
          value: this.leaves[index].value.toString(),
          nextIndex: this.leaves[index].nextIndex.toString(),
          nextValue: this.leaves[index].nextValue.toString(),
        })}`
      );
    } else if (this.cachedLeaves[index]) {
      log(
        `this.cachedLeaves[${index}]: ${JSON.stringify({
          value: this.cachedLeaves[index].value.toString(),
          nextIndex: this.cachedLeaves[index].nextIndex.toString(),
          nextValue: this.cachedLeaves[index].nextValue.toString(),
        })}`
      );
    } else {
   ...
   * @returns Empty promise.
   */
  private async appendLeaf(leaf: bigint): Promise<void> {
    const newValue = leaf;
    ...
    log(
      `before append, previousLeaf's index: ${JSON.stringify(indexOfPrevious)}`
    );
    log(
      `before append, previousLeaf: ${JSON.stringify({
        value: previousLeafCopy.value.toString(),
        nextIndex: previousLeafCopy.nextIndex.toString(),
        nextValue: previousLeafCopy.nextValue.toString(),
      })}`
    );
    ...
    log(
      `newLeaf: ${JSON.stringify({
        value: newLeaf.value.toString(),
        nextIndex: newLeaf.nextIndex.toString(),
        nextValue: newLeaf.nextValue.toString(),
      })}`
    );
    this.cachedLeaves[Number(indexOfPrevious.index)] = previousLeafCopy;
    log(
      `after append, previousLeaf: ${JSON.stringify({
        value: previousLeafCopy.value.toString(),
        nextIndex: previousLeafCopy.nextIndex.toString(),
        nextValue: previousLeafCopy.nextValue.toString(),
      })}`
  ...
  public async init(prefilledSize: number): Promise<void> {
    ...
    for (let i = 1; i < prefilledSize; i++) {
      await this.appendLeaf(BigInt(i));
    }
    ...
    const promise = new Promise<void>((resolve, reject) => {
      this.db
        ...
        .on('data', function (data) {
          ...
          log(
            `values[${index}]: {value: ${values[index].value}, nextIndex: ${values[index].nextIndex}, nextValue: ${values[index].nextValue} }`
          );
        })
        .on('close', function () {})
        .on('end', function () {
  ...
  private async commitLeaves(): Promise<void> {
    ...
    for (const key of keys) {
      ...
      log(
        `  key: ${index}, value: LeafData{"value": ${value}, nextIndex: ${nextIndex}, nextValue:${nextValue}}`
      );
      batch.put(
        indexToKeyLeaf(this.getName(), BigInt(index)),
        encodeTreeValue(this.cachedLeaves[index])
      );
  ...
  private async _updateLeaf(leaf: bigint, index: bigint) {
   ...
   */
  public async updateLeafWithNoValueCheck(
    leaf: LeafData,
    index: bigint
  ): Promise<void> {
    let encodedLeaf;
