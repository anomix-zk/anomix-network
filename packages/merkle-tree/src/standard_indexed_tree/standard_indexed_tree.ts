...
export interface PreimageFactory {
   ...
   */
  fromLeaf(
    leaf: IndexedTreeLeaf,
    nextKey: bigint,
    nextIndex: bigint
  ): IndexedTreeLeafPreimage;
  /**
   * Creates a new preimage from a Uint8Array.
   * @param buffer - Uint8Array to create a preimage from.
   */
  fromUint8Array(buffer: Uint8Array): IndexedTreeLeafPreimage;
  /**
...
export const buildDbKeyForPreimage = (name: string, index: bigint) => {
  return `${name}:leaf_by_index:${index.toString(16)}`;
};

export const buildDbKeyForLeafIndex = (name: string, key: bigint) => {
  return `${name}:leaf_index_by_leaf_key:${key.toString(16)}`;
};
...
export interface LeafFactory {
  ...
  /**
   * Creates a new leaf from a Uint8Array.
   * @param buffer - Uint8Array to create a leaf from.
   */
  fromUint8Array(buffer: Uint8Array): IndexedTreeLeaf;
}
...
function getEmptyLowLeafWitness<N extends number>(
  ...
  return {
    ...
    index: 0n,
    siblingPath: new SiblingPath(
      treeHeight,
      Array(treeHeight).fill(int256ToUint8ArrayLE(0n))
    ),
  };
}
...
export class StandardIndexedTree extends TreeBase implements IndexedTree {
  #snapshotBuilder = new IndexedTreeSnapshotBuilder(
    this.db,
    this,
  ...
  public constructor(
    ...
    this.log = consola.withTag('aztec:standard-indexed-tree');
  }
   ...
   */
  appendLeaves(_leaves: bigint[]): Promise<void> {
  ...
  > {
    ...
    let lowLeafPreimage =
      lowLeafIndex !== undefined
        ? await this.getDbPreimage(lowLeafIndex)
        : undefined;
    ...
    if (includeUncommitted) {
      ...
      if (cachedLowLeafIndex !== undefined) {
        ...
        if (
          !lowLeafPreimage ||
          cachedLowLeafPreimage.getKey() > lowLeafPreimage.getKey()
        ) {
  ...
  private async getDbPreimage(
    ...
    const dbPreimage = await this.db
      ...
      .then((data) => this.leafPreimageFactory.fromUint8Array(data))
      .catch(() => undefined);
    return dbPreimage;
  }
  ...
  private getCachedPreimage(
    index: bigint
  ): IndexedTreeLeafPreimage | undefined {
  ...
  public async init(prefilledSize: number): Promise<void> {
    ...
    leaves[prefilledSize - 1] = this.leafPreimageFactory.fromLeaf(
      leaves[prefilledSize - 1].asLeaf(),
      0n,
      0n
    );
  ...
  private async commitLeaves(): Promise<void> {
    ...
    for (const key of keys) {
      ...
      batch.put(
        buildDbKeyForLeafIndex(this.getName(), leaf.getKey()),
        int256ToUint8ArrayLE(index, 32)
      );
    }
    await batch.write();
    this.clearCachedLeaves();
  ...
  >(
    ...
    const emptyLowLeafWitness = getEmptyLowLeafWitness(
      this.getDepth() as TreeHeight,
      this.leafPreimageFactory
    );
    // Accumulators
    const lowLeavesWitnesses: LowLeafWitnessData<TreeHeight>[] = leaves.map(
      () => emptyLowLeafWitness
    );
    const pendingInsertionSubtree: IndexedTreeLeafPreimage[] = leaves.map(() =>
      this.leafPreimageFactory.empty()
    );

    // Start info
    const startInsertionIndex = this.getNumLeaves(true);

    const leavesToInsert = leaves.map((leaf) =>
      this.leafFactory.fromUint8Array(leaf)
    );
    const sortedDescendingLeafTuples = leavesToInsert
      .map((leaf, index) => ({ leaf, index }))
      .sort((a, b) => Number(b.leaf.getKey() - a.leaf.getKey()));
    const sortedDescendingLeaves = sortedDescendingLeafTuples.map(
      (leafTuple) => leafTuple.leaf
    );
    ...
    for (let i = 0; i < leavesToInsert.length; i++) {
      ...
      const indexOfPrevious = await this.findIndexOfPreviousKey(
        newLeaf.getKey(),
        true
      );
      if (indexOfPrevious === undefined) {
        return {
          ...
          newSubtreeSiblingPath: await this.getSubtreeSiblingPath(
            subtreeHeight,
            true
          ),
        };
      ...
      const lowLeafPreimage = (await this.getLatestLeafPreimageCopy(
  ...
  private async encodeAndAppendLeaves(
    preimages: IndexedTreeLeafPreimage[],
    hash0Leaf: boolean
  ): Promise<void> {
    const startInsertionIndex = this.getNumLeaves(true);

    const hashedLeaves = preimages.map((preimage, i) => {
      this.cachedLeafPreimages[(startInsertionIndex + BigInt(i)).toString()] =
        preimage;
