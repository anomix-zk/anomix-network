...
export class SiblingPath<N extends number> {
   ...
   * @param poseidon - Implementation of a hasher interface using the Poseidon hash.
   * @returns A sibling path hashed up from a zero element.
   */
  public static ZERO<N extends number>(size: N, zeroElement: Buffer, poseidon: PoseidonHasher): SiblingPath<N> {
    const bufs: Buffer[] = [];
   ...
   */
  public toFieldArray(): Field[] {
    return this.data.map(buf => Field(uint8ArrayToBigInt(buf)));
  }
