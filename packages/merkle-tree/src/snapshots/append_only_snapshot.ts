...
export class AppendOnlySnapshotBuilder implements TreeSnapshotBuilder {
  ...
  async getSnapshot(block: number): Promise<TreeSnapshot> {
    ...
    if (typeof meta === 'undefined') {
      throw new Error(
        `Snapshot for tree ${this.tree.getName()} at block ${block} does not exist`
      );
    }
...
class AppendOnlySnapshot implements TreeSnapshot {
  ...
  async getLeafValue(index: bigint): Promise<bigint | undefined> {
    ...
    const blockNumber = await this.#getBlockNumberThatModifiedNode(
      leafLevel,
      index
    );
  ...
  async #getHistoricalNodeValue(level: number, index: bigint): Promise<bigint> {
    const blockNumber = await this.#getBlockNumberThatModifiedNode(
      level,
      index
    );
  ...
  async #getBlockNumberThatModifiedNode(
    level: number,
    index: bigint
