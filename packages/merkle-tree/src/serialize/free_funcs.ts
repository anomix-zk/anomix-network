...
import { toBigIntBE, toBufferBE, uint8ArrayToBigInt } from '../utils';
...
export function deserializeField(buf: Buffer, offset = 0) {
  const adv = 32;
  return { elem: Field(uint8ArrayToBigInt(buf.subarray(offset, offset + adv))), adv };
}
 ...
 */
export function to2Fields(buf: Buffer): [Field, Field] {
  if (buf.length !== 32) {
  ...
  return [Field(uint8ArrayToBigInt(buf1)), Field(uint8ArrayToBigInt(buf2))];
}
