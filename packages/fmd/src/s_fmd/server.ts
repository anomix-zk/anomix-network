...
export class Server {
    ...
    run(n: number) {
        ...
        let tag = generateEntangledTag([reciever1.pk, reciever2.pk], n);
        console.timeEnd("generateEntangledTag");

        const tagBytes = tag.toBytes();
        console.log("tagBytes len: ", tagBytes.length);
        ...
        let p = tag.falsePositiveProbability();
